...
function langHeader(language: "ar" | "en"): string {
  if (language === "ar") {
    return `اكتب الرد بالكامل باللغة العربية الفصحى المبسطة. يُسمح باستخدام المصطلحات المهنية والمهارات وأسماء الأدوات بالإنجليزية عند الحاجة (مثل React, LinkedIn, Product Manager).
